...
Les tests CI utilisent les wrappers stricts Bash, donc `bin/admin-node` doit exister. `make build-admin-node` et le role Ansible `base` appellent tous les deux `scripts/build-admin-node.sh`.

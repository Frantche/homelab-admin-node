...
| --- | --- | --- |
| `cloudflare.tunnel_id` | yes | Tunnel ID used by local ingress mode. |
| `cloudflare.tunnel_token` | yes | Token injected into the cloudflared container when local ingress mode is not selected. |
...
| `cloudflare.credentials_json` | yes | Tunnel-specific credentials generated by `cloudflared tunnel create`, required by local ingress mode. |

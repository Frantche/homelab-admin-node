...
keeps routes available when the LAPI cannot be reached. The middleware is
attached to both HTTPS entrypoints, so it is automatically prepended to every
router, including routes added later. The LAPI route is safe to protect because
...
See [Cloudflare Tunnel Credentials](../cloudflare-tunnel-credentials/) for the locally-managed tunnel creation procedure, required permissions, SOPS storage, and public DNS routes.

Store tunnel identifiers, account IDs, API tokens, and credentials JSON in encrypted secrets.

| Variable | Secret | Purpose |
| --- | --- | --- |
| `cloudflare.tunnel_id` | yes | Tunnel UUID passed to the locally-managed cloudflared process. |
| `cloudflare.account_id` | yes | Account identifier reserved for Cloudflare API workflows; it is not consumed by the current deployment role. |
| `cloudflare.dns_api_token` | yes | DNS API token used for managed tunnel CNAME records and by Traefik ACME DNS challenge when available. |
| `cloudflare.credentials_json` | yes | Tunnel-specific credentials generated by `cloudflared tunnel create` and mounted read-only in the container. |
| `cloudflare.origin_network.subnet` | no | Dedicated internal Docker subnet between cloudflared and Traefik. Defaults to `172.31.255.0/29`. |
| `cloudflare.origin_network.traefik_ip` | no | Fixed Traefik origin address. Defaults to `172.31.255.2`. |
| `cloudflare.origin_network.cloudflared_ip` | no | Fixed connector address trusted for forwarded client headers. Defaults to `172.31.255.3`. |
| `cloudflare.dns.enabled` | no | Enables idempotent, exact CNAME management for the published tunnel hostnames. Defaults to `false`. |
| `cloudflare.dns.zone` | no | Cloudflare DNS zone containing every managed hostname. |

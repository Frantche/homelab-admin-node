...
grep -qF -- '- traefik-openbao' "$REPO_ROOT/stacks/openbao/compose.yaml.j2"
grep -qF -- '- openbao-metrics' "$REPO_ROOT/stacks/openbao/compose.yaml.j2"
grep -qF -- '- "{{ service_domains.keycloak }}"' "$REPO_ROOT/stacks/traefik/compose.yaml.j2"

if grep -qF -- '- admin-edge' "$REPO_ROOT/stacks/openbao/compose.yaml.j2"; then
